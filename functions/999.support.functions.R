# Script name: 999.support.functions.R
# Purpose: Principal Component Analysis
# Used packages: subselect
# Author: prof. Massimo Bilancia
# Date: 18-03-2014

library(subselect)

svd.pc <- function(train, test, k, trainset = deparse(substitute(train)), center = TRUE, 
    scale = FALSE, plot = TRUE, save.plot = FALSE, fig.path = figures.path) {
    ...
    ## %*% V_train, dove 'X_test' è la matrice di dimensione che contiene le metriche
    ## nel test set V è la matrice dei loadings di dimensione (19x19) ottenuta da
    ## 'svd(train)$v' mediante la SVD nel training set utilizzato ATTENZIONE: BISOGNA
    ## PROIETTARE RISPETTO ALLO STESSO SISTEMA DI COORDINATE USATO PER LE PC NEL
    ## TRAINING SET
    if (center) {
        ...
        if (save.plot) {
            if (center) {
                filename.postscript <- paste(fig.path, "metric.pc.centered.", trainset, 
                  ".kubios.lambda", lambda, ".", artefact, ".eps", sep = "")
                if (center & scale) {
                  filename.postscript <- paste(fig.path, "metric.pc.centered.scaled.", 
                    trainset, ".kubios.lambda", lambda, ".", artefact, ".eps", sep = "")
                }
            }
            postscript(file = filename.postscript, width = 22, height = 22, paper = "special", 
                pointsize = 22)
        }
